...
export const AuthScreen: React.FC<AuthScreenProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setFormError(err?.message || 'Error al conectar con Google.');
      setIsGoogleLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-[#0B132B] flex flex-col justify-center items-center p-4 sm:p-6 text-slate-100 selection:bg-[#147DF0] selection:text-white relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#1C2A4A]/80 backdrop-blur-xl border border-white/10 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 relative z-10 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="grid grid-cols-2 gap-1 p-1 bg-slate-900/90 border border-slate-800 rounded-xl mb-4">
          <button
            type="button"
            onClick={() => {
              setMode('login');
        ...
        {onSignInWithGoogle && (
          <div className="space-y-3">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-2xl bg-white hover:bg-slate-100 active:scale-[0.99] text-slate-900 text-sm font-bold shadow-lg shadow-black/20 border border-white/20 transition-all disabled:opacity-50 cursor-pointer group"
            >
              {isGoogleLoading ? (
                <div className="w-5 h-5 border-2 border-slate-700 border-t-transparent rounded-full animate-spin shrink-0" />
              ) : (
            ...
            {/* Divisor con estilo elegante */}
            <div className="relative flex items-center justify-center pt-1 pb-1">
              <div className="border-t border-white/10 w-full" />
              <span className="bg-[#1C2A4A] px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider shrink-0">
                {mode === 'login' ? 'o ingresa con tu cédula' : 'o regístrate con tu cédula'}
              </span>
              <div className="border-t border-white/10 w-full" />
            </div>
          </div>
